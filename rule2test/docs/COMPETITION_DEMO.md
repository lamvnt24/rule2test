# Kịch bản demo 5 phút — testcase và luật độc lập

| Thời gian | Thao tác và điểm cần trình bày |
|---|---|
| 0:00–1:00 | Import testcase thuần, preview ô nguồn, xử lý dòng mơ hồ. |
| 1:00–2:00 | Gõ luật cũ/mới, Interpret rules, kiểm tra và Confirm rules. Pattern là offline; chỉ gọi là AI khi dùng provider thật. |
| 2:00–3:00 | Compare: TC002 đổi DENY → ALLOW, bổ sung tuổi 65/66, chỉ ra Not linked. |
| 3:00–4:00 | Duyệt tất cả test hiện hành, Finalize, execute, evidence, gate GO. |
| 4:00–5:00 | Reopen/review/finalize, cài lỗi Boundary, chạy và tạo evidence mới, gate NO-GO. |

Nếu không chạy live được, mở `data/generated/suite-demo-capture/replay.html`. Đây là ảnh chụp luồng pattern + mock SUT, không phải kết quả AI live hay hệ thống bảo hiểm thật. Số testcase phụ thuộc đầu vào. Tài liệu cũ được giữ tại `COMPETITION_DEMO-LEGACY.md`.
