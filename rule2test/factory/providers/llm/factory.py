...
from .patterns import PatternRuleProvider
...
def configured_provider(env=None):
    ...
    if name=="pattern":return PatternRuleProvider()
    if name=="ollama":return OllamaLLMProvider(env.get("RULE2TEST_EXTRACTION_MODEL",""))
    raise ConfigurationError("Extraction provider must be mock, pattern or ollama; no automatic fallback")
